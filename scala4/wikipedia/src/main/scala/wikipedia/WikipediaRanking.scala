...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("Wikipedia text app")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((cnt, article) => cnt + (if (article.mentionsLanguage(lang)) {
      1
    } else {
      0
    }), _ + _)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(t => (t._1, t._2.count(a => true))).sortBy(-_._2).collect().toList
  }
